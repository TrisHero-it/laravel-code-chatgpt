...
@section('title', 'Thêm đơn hàng Identity V')
@section('content')
<div class="container mt-5">
    ...
    <div class="container mt-5">
        <h3>Thêm đơn hàng Identity V</h3>

        <form action="{{ route('identity-order.store') }}" method="post" id="identityOrderForm">
            @csrf
            <div class="form-group mt-3">
                ...
                <textarea class="form-control" id="order_data" name="order_data" rows="6"
                    placeholder="Dán thông tin đơn hàng vào đây (ví dụ:&#10;Mã ĐH: 1296509&#10;Sản phẩm: 8th Anniversary Discount Package II Nạp Identity V Global Chỉ Cần ID x 1&#10;UID Identity V: 73432255&#10;Game Server: asia&#10;Ngày mua: 00:53:20 25/07/2026)" required></textarea>
            ...
            <div class="d-flex" style="gap: 8px">
                ...
                <a href="{{ route('identity-order.index') }}" class="btn btn-secondary mt-3">Quay lại</a>
            </div>
        </form>
    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function parseOrderData(text) {
                ...
                // Parse UID: "UID Identity V: 73432255"
                const uidMatch = text.match(/UID Identity V:\s*(\d+)/i) ||
                    text.match(/UID:\s*(\d+)/i);
                if (uidMatch) {
                    result.uid = uidMatch[1].trim();
                }

                // Parse Server: "Game Server: asia"
                const serverMatch = text.match(/Game Server:\s*(.+?)(?:\n|$)/i);
                if (serverMatch) {
                ...
                // Parse Product Name: "Sản phẩm: 8th Anniversary Discount Package II Nạp Identity V Global Chỉ Cần ID x 1"
            ...
            function updatePreview(parsed) {
                ...
                const serverLi = document.getElementById('preview_server_li');
                if (parsed.server) {
                    serverLi.style.display = '';
                    document.getElementById('preview_server').textContent = parsed.server;
                } else {
                    serverLi.style.display = 'none';
                }
            ...
            continueAddBtn.addEventListener('click', function() {
                ...
                document.getElementById('identityOrderForm').submit();
            });

            // Validate trước khi submit
            document.getElementById('identityOrderForm').addEventListener('submit', function(e) {
                const orderId = orderIdInput.value.trim();
                const uid = uidInput.value.trim();
                const productId = productIdInput.value.trim();
