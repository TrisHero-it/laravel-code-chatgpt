...
@section('title', 'Sửa đơn hàng Identity V')
@section('content')
<div class="container mt-5">
    <h3>
        Sửa đơn hàng Identity V
    </h3>
    ...
    <form action="{{ route('identity-order.update', ['identity_order' => $order['id']]) }}" method="post" id="identityOrderForm" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="form-group mt-3">
            <label for="server">Server <span class="text-muted">(Tùy chọn)</span></label>
            <input type="text" class="form-control" id="server" name="server" placeholder="Nhập Server (ví dụ: asia)" value="<?php echo htmlspecialchars($order['server'] ?? '') ?>" style="max-width: 200px;">
        </div>
        <div class="form-group mt-3">
        ...
        <div class="d-flex" style="gap: 8px">
            <button type="submit" class="btn btn-primary mt-3" id="updateIdentityOrderButton">Cập nhật đơn hàng</button>
